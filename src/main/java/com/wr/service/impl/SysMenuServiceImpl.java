...
import com.wr.common.constants.UserConstants;
...
public class SysMenuServiceImpl extends ServiceImpl<SysMenuMapper, SysMenuPo> implements ISysMenuService {
    ...
    public boolean install(AddSysMenuDto addSysMenuDto) {
        if (sysMenuMapper.insert(BeanUtil.beanToBean(addSysMenuDto,new SysMenuPo())) > 0){
            return true;
        }
        return false;
    }
    ...
    public boolean update(UpSysMenuDto upSysMenuDto) {
        if (sysMenuMapper.updateById(BeanUtil.beanToBean(upSysMenuDto,new SysMenuPo())) > 0){
            return true;
        }
        return false;
    }
